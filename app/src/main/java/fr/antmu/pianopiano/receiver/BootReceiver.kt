...
class BootReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action == Intent.ACTION_BOOT_COMPLETED) {
            if (BuildConfig.DEBUG) Log.d("BootReceiver", "Boot completed - Starting initialization")
            ...
            try {
                ...
                if (BuildConfig.DEBUG) Log.d("BootReceiver", "BootInitService started successfully")
            } catch (e: Exception) {
